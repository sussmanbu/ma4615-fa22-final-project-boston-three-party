...
```
The above line graph shows barley production in United States from 1990 to 2021. Over the year last 20 years,
we can see a clear net downward trend. 




```{r,1}
barley_co2_data %>% ggplot(aes(x = `Area harvested`, y = (CO2_emissions_kt))) + geom_point()
barley_co2_data %>% ggplot(aes(x = `Production`, y = (CO2_emissions_kt))) + geom_point()
barley_co2_data %>% ggplot(aes(x = `Area planted`, y = (CO2_emissions_kt))) + geom_point()
barley_co2_data %>% ggplot(aes(x = `Yield per acre`, y = (CO2_emissions_kt))) + geom_point()

```
We explored different variables and factors to conclude that there seems to be no relationship between the co2 emission and any of the barley variables. 
