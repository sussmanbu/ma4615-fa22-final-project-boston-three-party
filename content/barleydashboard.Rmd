...
g <- ggplot(barley, aes(x = Time, y = Production, color = CO2_emissions_kt,
  text = paste("Year:", Time, "<br>", "Production Output:", Production, "<br>", "Yield per Acre:", barley$`Yield per acre`))) +
  geom_point(position = "jitter")
ggplotly(g, tooltip = "text")
```


### Chart 2 Animation Barley
```{r}
...
barley %>% select(Time, CO2_emissions_kt, Production) %>%
  #filter(row_number() %% 20 == 0) %>%
  formattable(list(displ = color_tile("white", "orange"),
    area(col = c(CO2_emissions_kt)) ~ normalize_bar("green", 0.2),
