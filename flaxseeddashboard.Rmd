...
flax <- flaxseed_co2_data
#corn <- read_csv(here::here("dataset", "corn_co2_data"))


g <- ggplot(flaxseed_co2_data, aes(x = Time, y = Production, color = CO2_emissions_kt.x,
  text = paste("Year:", Time, "<br>", "Production Output:", Production, "<br>", "Yield per Acre:", corn$`Yield per acre`))) +
  geom_point(position = "jitter")
ggplotly(g, tooltip = "text")
```


### Chart 2 Animation flax
```{r}
...
gg <- ggplot(flax,
  aes(x = Time, y = Production,
    ...
    size = flax$`Yield per acre`, 
      text = paste("Year:", Time, "<br>", "Production Output:", Production, "<br>", "Yield per acre:", GDP_growth))) +
